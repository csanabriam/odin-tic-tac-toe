const tictactoe = (() => {
  // Contains all submodules and functions to control the game.
  // The module returns the function to initialize the game.
  ...
  const gameboard = (() => {
    ...
    let grid = [
      ...
      ['', '', ''],
      ['', '', ''],
    ]; // the gameboard is a 3x3 matrix
    let turnOfPlayer = 0; // 0 for player1 and 1 for player2
    ...
    const checkIfTurnOfCPU = () => {
      // Determine whether it's the CPU turn to play,
      // if so calls the function to make the CPU play.
    ...
    const _play = (event) => {
      // Mark an empty space with O or X,
      // if there is no winner hand the turn to the other player.
      // This function is called when a player clicks in his turn on an empty space.
      const row = +event.composedPath()[0].getAttribute('row');
      const column = +event.composedPath()[0].getAttribute('column');
      if (!grid[row][column]) {
    ...
    const _cpuPlays = () => {
      // Mark an empty space with O or X, based on the AI strategy,
      // if there is no winner, hands the turn to the other player.
      // This function is called to make the CPU play in its turn.
      if (!thereIsAWinner) {
        const [row, column] = ai.cpuPlays(turnOfPlayer, turn);
        if (!grid[row][column]) {
          grid[row][column] = turnOfPlayer ? 'O' : 'X';
          spaces[row * 3 + column].textContent = turnOfPlayer ? '⭕' : '❌';
          spaces[row * 3 + column].classList.add('marked');
    ...
    const markWinningSpaces = (toMark) => {
      ...
      for (const [row, column] of toMark) {
    ...
    const returnGrid = () => grid; // Return the gameboard to be read by other modules.
    const checkIfFull = () => {
  ...
  const display = (() => {
    // Contains the functions to controls the html elements of the display:
    // -the names of the players
    // -the sign announcing the winner.
    ...
    const _setNewPlayersNames = () => {
      // Read the names of the players, determine whether one of them is the CPU.
  ...
  const ai = (() => {
    ...
    const cpuPlays = (turnOfPlayer, turn) => {
      ...
      const player = turnOfPlayer ? 'player2' : 'player1';
      switch (game[player].CPUlevel) {
        case 1:
          // The CPU plays randomly except when he can make a winning choice.
          const possibleRandomDecisions = [];
          const copyOfGrid = Array.from(gameboard.returnGrid());
          ...
          for (const space of possibleRandomDecisions) {
            // Checks if there is a winning choice and if so, chooses it.
            const column = space % 3;
            const row = (space - column) / 3;
            const newPhantomGrid = [];
            for (let i = 0; i < 3; i++) {
            ...
            const phantomGrid = {
              ...phantomGridProto,
              grid: newPhantomGrid,
              phantomTurnOfPlayer: turnOfPlayer,
            };
          ...
          const cpuChooses = Math.floor(
            possibleRandomDecisions.length * Math.random(),
          );
          const cpuPlaysIn = possibleRandomDecisions[cpuChooses];
          const randomColumn = cpuPlaysIn % 3;
          randomRow = (cpuPlaysIn - randomColumn) / 3;
          return [randomRow, randomColumn];
          break;
        case 3:
          // The CPU plays so that it minimizes the chances of loosing according to the weights
          // assigned to each possible choice by the function weightPossibleMoves2.
          // It's an optimal choice.
          const [possibleDecisions1, decisionsDamage1, decisionsAdvantage1] = ai.weightPossibleMoves(
            Array.from(gameboard.returnGrid()),
            turnOfPlayer,
            turnOfPlayer,
            turn,
          );
          const [possibleDecisions2, decisionsDamage2] = ai.weightPossibleMoves2(
            Array.from(gameboard.returnGrid()),
            turnOfPlayer,
            turnOfPlayer,
            turn,
          );
          const min2 = Math.min(...decisionsDamage2);
          const goodDecisions2 = getAllIndexes(decisionsDamage2, min2);
          const goodDecisionsDamage = goodDecisions2.map(
            (i) => decisionsDamage1[i],
          );
          const min1 = Math.min(...goodDecisionsDamage);
          const bestDecisions1 = getAllIndexes(decisionsDamage1, min1);
          const bestDecisionsAdvantage1 = bestDecisions1.map(
            (i) => decisionsAdvantage1[i],
          );
          const bestPossibleDecisions1 = bestDecisions1.map(
            (i) => possibleDecisions1[i],
          );
          const optimalSpace = bestPossibleDecisions1[
            bestDecisionsAdvantage1.indexOf(
              Math.max(...bestDecisionsAdvantage1),
            )
          ];
          const optimalColumn = optimalSpace % 3;
          const optimalRow = (optimalSpace - optimalColumn) / 3;

          return [optimalRow, optimalColumn];
          break;
        default:
          //  The CPU plays so that it minimizes the chances of loosing according to the weights
          //  assigned to each possible choice by the function weightPossibleMoves.
          const [possibleDecisions, decisionsDamage, decisionsAdvantage] = ai.weightPossibleMoves(
            Array.from(gameboard.returnGrid()),
            turnOfPlayer,
            turnOfPlayer,
            turn,
          );
          const min = Math.min(...decisionsDamage);
          const bestDecisions = getAllIndexes(decisionsDamage, min);
          const bestPossibleDecisions = bestDecisions.map(
            (i) => possibleDecisions[i],
          );
          const bestDecisionsAdvantage = bestDecisions.map(
            (i) => decisionsAdvantage[i],
          );
          const space = bestPossibleDecisions[
            bestDecisionsAdvantage.indexOf(
              Math.max(...bestDecisionsAdvantage),
            )
          ];
          const column = space % 3;
          const row = (space - column) / 3;
    ...
    const phantomPlays = (grid, requester, phantomTurnOfPlayer) => {
      // Get all possible ramifications from a given phantom gameboard.
      // It's an inductive function that calls itself after making a choice
      // to get all possible ramification from it.
      let children = [];
      ...
      for (let space = 0; space < 9; space++) {
        ...
        if (!grid[row][column]) {
          ...
          if (
            !phantomGrid.checkIfPhantomWinner()
            && !phantomGrid.checkIfFull()
          ) {
            children = children.concat(
              phantomPlays(
                phantomGrid.grid,
                requester,
                phantomGrid.phantomTurnOfPlayer,
              ),
            );
          } else {
    ...
    const weightPossibleMoves = (
      grid,
      requester,
      phantomTurnOfPlayer,
      turnNumber,
    ) => {
      // Calculate how likely is that you could loose or win for each single choice
      // available in a phantom gameboard.
      const possibleDecisions = [];
      const decisionsDamage = [];
      const decisionsAdvantage = [];
      ...
      for (const space of possibleDecisions) {
        const column = space % 3;
        const row = (space - column) / 3;
        const newPhantomGrid = [];
        for (let i = 0; i < 3; i++) {
        ...
        if (phantomGrid.checkIfPhantomWinner()) {
          ...
          const consequences = phantomPlays(
            phantomGrid.grid,
            requester,
            phantomGrid.phantomTurnOfPlayer,
          );
          const opponent = phantomTurnOfPlayer ? 0 : 1;
          for (const phantom of consequences) {
            damage
              += phantom.winnerIs === opponent
                ? factorial(9 - phantom.phantomDepth)
                  / factorial(9 - turnNumber)
                : 0;
            advantage
              += phantom.winnerIs === phantomTurnOfPlayer
                ? factorial(9 - phantom.phantomDepth) / factorial(9)
                : 0;
          }
        }
      ...
      for (const space of possibleDecisions) {
        ...
        if (phantomGrid.checkIfPhantomWinner()) {
          ...
          while (
            ...
            const [possibleDecisions, decisionsDamage, decisionsAdvantage] = weightPossibleMoves(
              Array.from(phantomGrid.grid),
              requester,
              phantomGrid.phantomTurnOfPlayer,
              turnNumber,
            );
            const min1 = Math.min(...decisionsDamage);
            const bestDecisions = getAllIndexes(decisionsDamage, min1);
            const bestPossibleDecisions = bestDecisions.map(
              (i) => possibleDecisions[i],
            );
            const bestDecisionsAdvantage = bestDecisions.map(
            ...
            const phantomColumn = space % 3;
            const phantomRow = (space - phantomColumn) / 3;
